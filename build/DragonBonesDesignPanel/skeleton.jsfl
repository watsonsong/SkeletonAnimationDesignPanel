...
function getAnimationXML(armatureXML, name, item, duration, isChildArmature)
{
	var xml = armatureXML[ANIMATION].(@name == name)[0];
	if(!xml)
	{
		var fadeInTime = isChildArmature?0:0.3;
		var loop = 1;
		var scale = 1;
		var tweenEasing = NaN;
...
function generateAnimation(item, mainFrame, layers, armatureXML, result, isChildArmature)
{
	...
	var animationXML = getAnimationXML(armatureXML, animationName, item, duration, isChildArmature);
		...
		while(frameID --)
		{
			if(keyFrameIdMark[frameID])
			{
				...
				if(_currentDom.selection.length)
				{
					...
					//_currentDom.enterEditMode("inPlace");
					//_currentDom.selectAll();
					//_currentDom.distributeToLayers();
					//_currentDom.breakApart();
					//_currentDom.exitEditMode();
...
dragonBones.generateArmature = function(armatureName, isChildArmature, newGenerate, isMergeLayersInFolder)
{
	...
	//
	if(isMergeLayersInFolder && canMergeLayersInFolder(item))
	{
	...
	{
		generateAnimation(item, mainFrame, layersFiltered, armatureXML, result, isChildArmature);
	}
